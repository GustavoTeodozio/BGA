...
export function VendedorProjects() {
  ...
  const { data: projects = [], isLoading } = useQuery({
    queryKey: ['vendedor', 'projects'],
    queryFn: async () => { const r = await api.get('/vendedor/projects'); return r.data; },
  });

  const { data: teamMembers = [] } = useQuery({
    queryKey: ['vendedor', 'team-members'],
    queryFn: async () => { const r = await api.get('/vendedor/team-members'); return r.data; },
  });

  const projetistas = teamMembers.filter((m: any) => m.role === 'PROJETISTA');

  const createMut = useMutation({
    mutationFn: (data: any) => api.post('/vendedor/projects', data),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['vendedor', 'projects'] }); closeModal(); },
    onError: (e: any) => setFormError(e.response?.data?.message || 'Erro ao salvar'),
  });

  const updateMut = useMutation({
    mutationFn: ({ id, data }: { id: string; data: any }) => api.patch(`/vendedor/projects/${id}`, data),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['vendedor', 'projects'] }); closeModal(); },
    onError: (e: any) => setFormError(e.response?.data?.message || 'Erro ao salvar'),
  });

  const deleteMut = useMutation({
    mutationFn: (id: string) => api.delete(`/vendedor/projects/${id}`),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['vendedor', 'projects'] }),
  });
  ...
  const counts = {
    ...
    ativos:    projects.filter((p: any) => !['FINALIZADO'].includes(p.status)).length,
    finalizados: projects.filter((p: any) => p.status === 'FINALIZADO').length,
  };
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <p className="text-gray-500 text-sm font-outer-sans">Projetos que você criou e direcionou para projetistas</p>
        <button
          onClick={openCreate}
          className="flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-semibold rounded-lg transition-colors font-outer-sans shadow whitespace-nowrap"
        >
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        {[
          ...
          { label: 'Em andamento', value: counts.ativos, color: 'text-blue-600' },
          { label: 'Finalizados', value: counts.finalizados, color: 'text-green-600' },
        ].map(s => (
          <div key={s.label} className="bg-white rounded-xl border border-gray-100 p-4 text-center">
            <p className={`text-2xl font-bold font-outer-sans ${s.color}`}>{s.value}</p>
            <p className="text-xs text-gray-400 font-outer-sans mt-0.5">{s.label}</p>
          </div>
        ))}
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 bg-white rounded-xl border border-dashed border-gray-200">
          ...
          <button onClick={openCreate} className="mt-3 text-green-600 text-sm font-semibold font-outer-sans hover:underline">
            Criar o primeiro
          </button>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {projects.map((p: any) => (
            ...
            >
              ...
              {p.clientName && <p className="text-xs text-gray-500 font-outer-sans mb-1">{p.clientName}</p>}

              <p className="text-xs text-gray-400 font-outer-sans">
                Projetista:{' '}
                <span className={`font-medium ${p.designer ? 'text-purple-600' : 'text-amber-500'}`}>
                  {p.designer?.name || 'Não atribuído'}
                </span>
              </p>
              ...
              <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-50">
                ...
                <button
                  onClick={async (e) => {
                    e.stopPropagation();
                    const ok = await confirm({ title: 'Excluir projeto', message: 'Excluir este projeto?', confirmText: 'Excluir' });
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-green-300 font-outer-sans"
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1 font-outer-sans">
                  Direcionar para Projetista
                </label>
                <select
                  value={form.designerId}
                  onChange={e => setForm(f => ({ ...f, designerId: e.target.value }))}
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-green-300 font-outer-sans bg-white"
                >
                  <option value="">Sem projetista atribuído</option>
                  {projetistas.map((m: any) => (
                    <option key={m.id} value={m.id}>{m.name}</option>
                  ))}
                </select>
                {projetistas.length === 0 && (
                  <p className="text-[10px] text-amber-500 mt-1 font-outer-sans">Nenhum projetista disponível no sistema ainda.</p>
                )}
              </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1 font-outer-sans">Descrição / Briefing</label>
                <textarea
                  rows={3}
                  value={form.description}
                  onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-green-300 font-outer-sans resize-none"
                  placeholder="Detalhes do projeto, materiais, referências, briefing do cliente..."
                />
              </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  type="submit"
                  disabled={isSaving}
                  className="flex-1 px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg text-sm font-semibold transition-colors font-outer-sans disabled:opacity-60"
                >

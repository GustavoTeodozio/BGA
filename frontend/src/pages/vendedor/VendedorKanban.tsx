...
const columns: { id: TaskStatus; title: string; color: string; gradient: string }[] = [
  { id: 'BACKLOG', title: 'Solicitado', color: 'bg-gradient-to-br from-gray-50 to-gray-100 border-gray-300', gradient: 'from-gray-500 to-gray-600' },
  { id: 'IN_PRODUCTION', title: 'Em Produção', color: 'bg-gradient-to-br from-blue-50 to-cyan-50 border-blue-300', gradient: 'from-blue-500 to-cyan-500' },
  { id: 'FOR_APPROVAL', title: 'Aprovação', color: 'bg-gradient-to-br from-yellow-50 to-amber-50 border-yellow-300', gradient: 'from-yellow-500 to-amber-500' },
  { id: 'SCHEDULED', title: 'Montagem', color: 'bg-gradient-to-br from-purple-50 to-pink-50 border-purple-300', gradient: 'from-purple-500 to-pink-500' },
  { id: 'PUBLISHED', title: 'Concluído', color: 'bg-gradient-to-br from-green-50 to-emerald-50 border-green-300', gradient: 'from-green-500 to-emerald-500' },
];

let touchGhost: HTMLElement | null = null;

export function VendedorKanban() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['vendedor', 'tasks'],
    queryFn: async () => {
      const response = await api.get('/vendedor/tasks?perPage=100');
  ...
  const updatePositionsMutation = useMutation({
    mutationFn: async (updates: { id: string; position: number; status: TaskStatus }[]) => {
      await api.post('/vendedor/tasks/positions', { updates });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['vendedor', 'tasks'] });
    },
  });
      ...
      <div className="mb-4 md:mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
          ...
          <button
            ...
            className="group px-4 py-2.5 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-xl font-semibold font-outer-sans shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 flex items-center justify-center gap-2 text-sm w-full sm:w-auto"
          >
        ...
        <div className="grid grid-cols-5 gap-2">
          {columns.map((col) => (
            <div key={col.id} className={`p-2 sm:p-3 rounded-xl ${col.color} border-2 transition-all duration-300 hover:shadow-md`}>
        ...
        {columns.map((column) => (
          <div
            ...
            className={`snap-start shrink-0 w-[260px] sm:w-[280px] lg:shrink lg:flex-1 lg:w-auto lg:min-w-0 rounded-2xl border-2 p-3 sm:p-4 min-h-[200px] md:min-h-[400px] transition-all duration-300 backdrop-blur-sm ${column.color} ${
              dragOverColumn === column.id ? 'shadow-2xl scale-[1.01] ring-4 ring-blue-400 ring-opacity-50' : 'shadow-sm hover:shadow-md'
            }`}
          >
            {/* Column Header */}

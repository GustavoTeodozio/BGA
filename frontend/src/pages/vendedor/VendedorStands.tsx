...
import { StandUpdatesManager } from '../admin/StandUpdatesManager';

export function VendedorStands() {
  const [selectedClientId, setSelectedClientId] = useState<string | null>(null);

  const { data: clients = [], isLoading } = useQuery<any[]>({
    queryKey: ['vendedor', 'clients'],
    queryFn: async () => (await api.get('/vendedor/clients')).data,
  });
      ...
      <div className="w-64 flex-shrink-0">
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden h-full flex flex-col">
          <div className="px-4 py-3 border-b border-gray-100">
            <h3 className="text-sm font-semibold text-gray-800 font-outer-sans">Meus Clientes</h3>
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-colors ${
                      active ? 'bg-green-50 border-l-2 border-green-700' : 'hover:bg-gray-50 border-l-2 border-transparent'
                    }`}
                  >
                    ...
                    <div className="min-w-0">
                      <p className={`text-sm font-semibold truncate font-outer-sans ${active ? 'text-green-800' : 'text-gray-700'}`}>
                        {name}
                      </p>
      ...
      <div className="flex-1 min-w-0">
        {selectedClientId ? (
          ...
          <div className="bg-white border border-gray-200 rounded-xl shadow-sm h-full flex flex-col items-center justify-center text-center p-8">
            ...
            <p className="text-xs text-gray-400 font-outer-sans mt-1">Escolha um cliente na lista para gerenciar as fotos do stand</p>
          </div>
        )}
      </div>
    </div>

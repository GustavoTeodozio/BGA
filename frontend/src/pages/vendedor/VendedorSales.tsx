  ...
  return (
    <div className="animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-6">
        ...
        <button onClick={() => { setForm({ clientName: '', companyName: '', value: '', description: '', status: 'FECHADA' }); setShowModal(true); }}
      ...
      {isLoading ? (
        <div className="flex items-center justify-center min-h-[300px]"><div className="inline-block animate-spin rounded-full h-10 w-10 border-b-2 border-green-600"></div></div>
      ) : sales.length === 0 ? (
            ...
            <div key={s.id} className="bg-white rounded-xl border border-gray-100 hover:shadow-lg transition-all duration-300 group">
              <div className="p-5">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium ml-2 ${STATUS_MAP[s.status]?.color}`}>
                    {STATUS_MAP[s.status]?.label}
                  </span>
                </div>
                <p className="text-2xl font-bold text-green-600 font-outer-sans mb-1">{formatCurrency(Number(s.value))}</p>

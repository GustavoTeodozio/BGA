...
import { BgaIA } from './BgaIA';
import { StandProgress } from './StandProgress';

const PAGE_TITLES: Record<string, string> = {
  '/client':                 'Dashboard',
  '/client/kanban':          'Meu Projeto',
  '/client/stand-progress':  'Acompanhamento do Stand',
  '/client/media':           'Mídias',
  '/client/ai':              'BGA IA',
};
...
const PAGE_ICONS: Record<string, JSX.Element> = {
  '/client':                <svg {...s}><path {...p} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>,
  '/client/kanban':         <svg {...s}><path {...p} d="M4 5a1 1 0 011-1h4a1 1 0 011 1v7a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM14 5a1 1 0 011-1h4a1 1 0 011 1v12a1 1 0 01-1 1h-4a1 1 0 01-1-1V5zM4 17a1 1 0 011-1h4a1 1 0 011 1v2a1 1 0 01-1 1H5a1 1 0 01-1-1v-2z" /></svg>,
  '/client/stand-progress': <svg {...s}><path {...p} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>,
  '/client/media':          <svg {...s}><path {...p} d="M7 4v16M17 4v16M3 8h4m10 0h4M3 12h18M3 16h4m10 0h4M4 20h16a1 1 0 001-1V5a1 1 0 00-1-1H4a1 1 0 00-1 1v14a1 1 0 001 1z" /></svg>,
  '/client/ai':             <svg {...s}><path {...p} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" /></svg>,
};
...
export function ClientDashboard() {
  ...
  const getPageInfo = () => {
    ...
    const match = Object.keys(PAGE_TITLES)
      .filter(k => k !== '/client' && path.startsWith(k))
      .sort((a, b) => b.length - a.length)[0];
    if (match) return { title: PAGE_TITLES[match], icon: PAGE_ICONS[match] };
    return { title: 'Dashboard', icon: PAGE_ICONS['/client'] };
  };
      ...
      <div className="flex-1 md:ml-60 relative min-w-0 flex flex-col">
        ...
        <header className="sticky top-0 z-30 border-b border-gray-200/80"
          ...
          <div className="flex items-center justify-between h-14 px-3 md:px-5 gap-3">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              {user && (
                <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-xl border border-red-100 bg-red-50/80">
                  <UserAvatar size={6} />
                  <span className="text-[13px] font-medium text-gray-700 font-outer-sans max-w-[120px] truncate">{user.name}</span>
                  <span className="text-[10px] text-red-400 font-outer-sans border-l border-red-200 pl-2">Cliente</span>
                </div>
              )}
        ...
        <main className="flex-1 relative py-5 px-3 md:py-8 md:px-6">
          <Routes>
            ...
            <Route path="stand-progress" element={<StandProgress />} />
            <Route path="media" element={<MediaLibrary />} />
            <Route path="ai" element={<BgaIA />} />
          </Routes>
        </main>
      </div>
    </div>

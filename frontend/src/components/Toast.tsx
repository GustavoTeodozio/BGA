...
export function Toast({ message, type, onClose, duration = 4000 }: ToastProps) {
  ...
  return (
    <div className="fixed top-4 right-4 z-[100] animate-slide-up">
      <div className={`flex items-center gap-3 px-5 py-4 rounded-2xl shadow-2xl border-2 ${getStyles()} min-w-[320px] max-w-md`}>
        <div className="flex-shrink-0">
          {getIcon()}
        </div>
        <p className="flex-1 font-semibold font-outer-sans text-sm">
          {message}
        </p>
        <button
          onClick={onClose}
